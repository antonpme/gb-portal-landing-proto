...
(function () {
  ...
  /* ============================================================
     ...
     Наблюдатель смотрит на КОЛОННУ и пишет её рост на .gbsp; правило
     в studio-panel.css поднимает пол только у открытого
     пристыкованного ящика. Петли нет по построению: пол меняет высоту
     ЯЩИКА, а наблюдают за его СОСЕДОМ, чей рост от ящика не зависит
     (в отличие от ловушки 23, где сторож читал состояние того же
     объекта, которым двигала рука).
     ============================================================ */
  ...
  function watchColumn(host) {
    ...
    if (!col) return;
    try {
      new ResizeObserver(function () { measureColumn(host); }).observe(col);
    } catch (e) {
