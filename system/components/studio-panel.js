/* ============================================================
   ...
   системный <gb-toggle> --inverse --small --spread (с 02.09, до того --s,
   gbppl-toggle-small-1: ступень ниже всех, см. makeSegments); семья
   --inverse родилась в toggle.css этой волной, и консоль её первый носитель
...
(function () {
  ...
  /* ============================================================
     РОСТ КОЛОННЫ — ПОЛ ДЛЯ ЯЩИКА (gbppl-panel-30-4)
     ------------------------------------------------------------
     Тон, со скрина страницы Sandboxes: на странице без групп ящик
     короче колонны, и плашки торчат ниже его нижней кромки —
     «читается сломанным».

     Число берётся ЗАМЕРОМ, а не константой: колонна = кнопка STUDIO
     плюс столько плашек, сколько объявили владельцы режимов, и
     страница без инспектора не получает их вовсе. 228 сегодняшних
     пикселей — следствие, а не мера, и писать его в файл значило бы
     соврать на первой же странице без comments.js.

     Наблюдатель смотрит на СОДЕРЖИМОЕ колонны и пишет его рост на
     .gbsp; правило в studio-panel.css поднимает пол только у
     открытого пристыкованного ящика.

     ПОЧЕМУ ИМЕННО ДЕТИ, А НЕ КОРОБКА (gbppl-panel-30-6). До этой
     волны мерилась сама колонна, и это было честно: она стояла ростом
     в своё содержимое. Теперь у открытого ящика она ТЯНЕТСЯ под него
     (слово Тона), и её коробка стала зависеть от ящика — а ящик от
     неё, через пол. Замер коробки замкнул бы кольцо: пол потянул бы
     ящик, ящик растянул бы колонну, растянутая колонна подняла бы пол.
     Дети не тянутся ни при каком состоянии (кнопка ростом в слово,
     бровка ростом в плашки), поэтому их сумма — то же число, что
     мерилось раньше, и петли в ней нет по построению. Рамки колонны
     читаются со стиля, а не пишутся цифрой: волосок объявлен один раз
     в её правиле.
     ============================================================ */
  function measureColumn(host) {
    var shell = host.querySelector('.gbsp');
    var col   = host.querySelector('.gbsp-tabcol');
    if (!shell || !col) return;
    var h = 0;
    for (var i = 0; i < col.children.length; i++) {
      h += col.children[i].getBoundingClientRect().height;
    }
    if (!h) return;
    var cs = getComputedStyle(col);
    h += (parseFloat(cs.borderTopWidth) || 0) + (parseFloat(cs.borderBottomWidth) || 0);
    /* ВВЕРХ, а не к ближайшему: пол обязан быть НЕ МЕНЬШЕ колонны, и
       округление вниз оставляло сотую долю пикселя свеса (замер:
       227.734 → 227.73). */
    shell.style.setProperty('--gbsp-colh', Math.ceil(h * 100) / 100 + 'px');
  }

  function watchColumn(host) {
    measureColumn(host);
    var col = host.querySelector('.gbsp-tabcol');
    if (!col) return;
    try {
      var obs = new ResizeObserver(function () { measureColumn(host); });
      /* Наблюдаем ДЕТЕЙ, а не колонну: её коробка теперь ходит за
         ящиком, и сторож на ней будил бы сам себя (та же болезнь, что
         в ловушке 23, только другим концом). */
      for (var i = 0; i < col.children.length; i++) obs.observe(col.children[i]);
    } catch (e) {
      /* Очень старый движок: плашки приезжают один раз при загрузке,
         и одного замера после них хватает. */
      window.addEventListener('resize', function () { measureColumn(host); });
    }
  }
  ...
  /* ============================================================
     ПРИНАДЛЕЖНОСТЬ ВИДА (gbppl-panel-sandbox-1, Тон 03.09)
     ------------------------------------------------------------
     Дословно, по скрину консоли на live/checkout.html?v=2:
     «Управление показывает неправильно. Этот чекаут не является
     частью лайва — он у нас в песочнице. Версия V2 — часть
     песочницы».

     Раздел до этой волны считался ОДНОЙ ПАПКОЙ адреса: всё внутри
     live/ — Live Prototype (panel-8). Папка честно отвечала, пока
     страница была одна; но у страницы есть версии, и вариант из
     реестра лежит в той же папке, что и Live, потому что это ТА ЖЕ
     страница с другим ключом. Папка сказала «Live» про то, что
     Live не является, и человек читал вид как аппрувленный.

     Правило общее и checkout в нём не назван: место берётся не из
     адреса, а из РЕЕСТРА. Выбран вариант (forPage дал current у
     любого из variants) — стоишь в Sandboxes; не выбран ни один —
     стоишь на Live Prototype, ровно как раньше. Заведут варианты
     портала или главной — правило застанет их готовым.

     ЭЛЕМЕНТЫ СЮДА НЕ ВХОДЯТ, и это не упущение. Постановка Тона
     01.09 (PROTOTYPE-MODEL-SPEC §1) держит их отдельно нарочно:
     «хедер из Live или Suggested прямо на Live-странице, чтобы не
     создавать отдельный прототип ради хедера». Live-страница с
     примеренным Suggested-хедером остаётся Live-страницей: элемент
     переключают НА ней, а не вместо неё. Переселять её в песочницу
     значило бы отменить смысл всей затеи. Версия страницы — другое
     дело: она и есть вся страница.
     ============================================================ */
  function variantHere(pageId, root) {
    var reg = window.GB_SANDBOXES;
    if (!pageId || !reg || typeof reg.forPage !== 'function') return null;
    var slice = reg.forPage(pageId, root);
    if (!slice) return null;
    var on = null;
    slice.variants.forEach(function (v) { if (v.current) on = v; });
    return on;
  }
      ...
      '<div class="gbsp-row">' +
        ...
        '<span class="gbsp-row__ctl">' +
          ...
          '<button class="gbsp-pick" type="button"' +
            ...
            /* СКОЛЬКО ЕЩЁ ЗА ЭТОЙ ДВЕРЬЮ (gbppl-panel-30-5). Слот
               пустой у всех строк, кроме той, за которой лежит НЕ ОДНА
               настройка: тогда строка печатает первую и говорит «+2»
               про остальные. Слот в разметке общий, потому что строка
               выбора в панели одна на всех, а наполнение у него сегодня
               одно — ряд Demo (paintDemoRow). */
            '<span class="gbsp-pick__more" hidden></span>' +
  ...
  /* ЗНАЧЕНИЕ СТРОКИ — ПЕРВАЯ ОСЬ И «+N» ЗА НЕЙ (gbppl-panel-30-5,
     Тон со скрина live\design-share.html: «дропдаун не может быть
     пустым, когда при нажатии раскрываются три опции. Нужно нормально
     с точки зрения UX»).

     ЧТО БЫЛО И ПОЧЕМУ ЭТО БЫЛА ОШИБКА. Строка печатала значение
     ТОЛЬКО когда выбор на странице ровно один, а при двух и более
     молчала: довод panel-sections-1 гласил, что перечислять два
     сценария в одной строке значит соврать про то, какой главный.
     Довод верен про ПЕРЕЧИСЛЕНИЕ и неверен про молчание: пустое поле
     с шевроном обещает пустоту, а за ним лежат три оси. Правильный
     ответ дома уже был — им живёт строка Scenario: первая ось словом,
     остальные счётом. Главной при этом никто не объявляется: «+2» —
     это «здесь есть ещё две», а не «те хуже».

     Порядок первой оси — порядок объявления страницей (addGroup):
     страница сама решает, что у неё главное, и консоль за неё не
     угадывает. */
  function paintDemoRow(host) {
    var row = protoRow(host, 'demo');
    var val = protoRowVal(host, 'demo');
    if (!val) return;
    var picks = host.__demoPicks || [];
    val.textContent = picks.length ? (picks[0]() || '') : '';
    var more = row && row.querySelector('.gbsp-pick__more');
    if (!more) return;
    var rest = Math.max(0, picks.length - 1);
  ...
  /* ВЫСОТА КАДРА = ВЫСОТА УСТРОЙСТВА (Тон 15.09, со скрина телефона
     высотой в весь монитор: «ты где-нибудь видел, чтобы у мобильных
     телефонов были такие длинные экраны?.. размер экрана мы делаем
     не только по ширине, но и по высоте, сохраняем пропорции»).
     До этого кадр всегда был высотой в окно минус хром сцены, и
     Mobile 390 на большом мониторе становился лентой 1:2.6, которой
     не бывает ни у одного телефона.

     Откуда числа. Обе портретные пары дом уже записал в ROTATE:
     390×844 (вьюпорт iPhone 12..16) и 768×1024 (iPad) — здесь они
     просто разворачиваются в высоту. Десктопы: 1280×800 — классика
     ноутбука 16:10; 1920×1080 — монитор 16:9; 2258×1270 — 16:9 от
     клиентского кастомного 2xl (2258 · 9/16 = 1270.1). У кастомной
     ширины из поля устройства нет, и высота честно остаётся прежней
     (окно минус хром, CSS-фолбек) — кроме повёрнутых 844 и 1024,
     которые ROTATE и эта таблица знают по имени.

     Кадр НЕ масштабируется под окно: пропорция дороже влезания, и
     сцена прокручивается (overflow: auto у .gbsp-stage), как стол,
     на котором лежит устройство длиннее столешницы. */
  var HEIGHTS = {
    '390': 844, '844': 390,
    '768': 1024, '1024': 768,
    '1280': 800, '1920': 1080, '2258': 1270
  };
  ...
  function mountDevice(host) {
    ...
    function build(width) {
      ...
      /* Та же поправка на волосок, что у ширины строкой выше: пресет
         обязан значить ровно ту высоту, которую внутри прочтёт
         window.innerHeight. Пустая строка отдаёт высоту CSS-фолбеку
         (окно минус хром) — это кастомные ширины без устройства. */
      var dh = HEIGHTS[String(width)];
      screen.style.height = dh ? (dh + 2) + 'px' : '';
  ...
  class GbStudioPanel extends HTMLElement {
    ...
    connectedCallback() {
      ...
      /* gbppl-panel-30-4: пол ящика по росту колонны. Ставится ПОСЛЕ
         всего остального, потому что плашки модов приезжают позже
         (inspect.js через whenDefined) — наблюдатель их и застанет. */
      watchColumn(this);
    }
  }
